...
local function create_keydb_connection()
    ...
    local scheme, host, port, path = keydb_url:match("^(%w+)://([^:/]+):?(%d*)(.*)$")
    
    if not host then
        ngx.log(ngx.ERR, "keydb_l3_cache: Invalid KeyDB URL format: ", keydb_url)
...
function _M.set(key, value, ttl)
    ...
    ttl = ttl or keydb_config.get_default_ttl()
    ttl = math.min(ttl, keydb_config.get_max_ttl())
