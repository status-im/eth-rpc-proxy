...
type KeyBuilder interface {
	// For single request
	Build(chain string, network string, req *models.JSONRPCRequest) (key string, paramsHash uint32)
	// For batch, returns per-item keys aligned by index
	BuildBatch(chain, network string, reqs []models.JSONRPCRequest) ([]string, []uint32)
}
